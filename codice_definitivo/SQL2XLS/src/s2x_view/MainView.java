...
import updatehandlers.ParseUpdate;
import utility.*;
import frame.*;
...
import createinserthandlers.ParseInsert;
...
public class MainView {
	...
	private void initialize() {
		...
		frmSqlToXlsx.setTitle("SQL to XLS");
		...
		frmSqlToXlsx.setBounds((width.intValue()/2)-215, (height.intValue()/2)-300, 430, 640);		
		
		frmSqlToXlsx.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frmSqlToXlsx.getContentPane().setLayout(null);
		
		JLabel lblSqlToXlsx = new JLabel("SQL to XLS");
		...
		panel_1.setBounds(31, 229, 359, 93);
		...
		JLabel lblXlsxFileStatus = new JLabel("XLS file status:");
		...
		jsp.setBounds(31, 375, 359, 173);
		frmSqlToXlsx.getContentPane().add(jsp);
		
		final JProgressBar progressBar = new JProgressBar();
		progressBar.setBounds(31, 333, 359, 14);
		...
		btnClearLog.setBounds(301, 552, 89, 23);
		frmSqlToXlsx.getContentPane().add(btnClearLog);
		...
		btnXlsxFile.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if(returnAction == 0)
				{
					...
					if(extPos != -1) {
					   String ext = fileName.substring(extPos,fileName.length());
					   if(ext.toLowerCase().compareTo(".xls") == 0) //comparazione estensione
						   correctExt = true;
					}
		...
		btnCreate.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				Utils.getInstance().setErrorsCreate(new Hashtable<Integer,String>());
				
				String filename = textFileName.getText();
				if(filename.equals(""))
				{
					filename="output";
					logPane.setText(logPane.getText()+getCurrentTime()+"Using default file name: output.xls\n");
				}
				try
				{
					ParseInsert.startParsing(sqlInput.getAbsolutePath(),outputDirectory,filename);
				}
				catch(Exception e)
				{
					e.printStackTrace();
					logPane.setText(logPane.getText()+getCurrentTime()+"Exception:"+e.toString()+"\n");				

				}
				progressBar.setValue(100);
				
				String errors = Utils.getInstance().getErrorsCreate().toString();
				if(errors.equals("{}"))
					errors = "No errors";
				
				logPane.setText(logPane.getText()+getCurrentTime()+"Error log: "+errors+"\n");	
				logPane.setText(logPane.getText()+getCurrentTime()+"CREATE parsing end\n");	
		...
		btnUpdate.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				ParseUpdate.startParsing(sqlInput.getAbsolutePath(),outputDirectory,xlsxInput.getAbsolutePath(),filename);
				...
				logPane.setText(logPane.getText()+getCurrentTime()+"Error log: "+errors+"\n");
				logPane.setText(logPane.getText()+getCurrentTime()+"UPDATE parsing end\n");
